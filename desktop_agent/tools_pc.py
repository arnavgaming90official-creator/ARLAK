...
@register("typeUrlInActiveBrowser")
def type_url_in_active_browser(args: Dict[str, Any]) -> Dict[str, Any]:
    url = args.get("url")
    if not url:
    ...
    try:
        ...
        # Type the URL
        pyautogui.typewrite(str(url), interval=0.01)
        
        # Press Enter
        pyautogui.press("enter")
        return {"result": f"Typed '{url}' into the active browser window."}
    except Exception as e:
        raise ToolError(f"Could not type URL via pyautogui: {e}")
