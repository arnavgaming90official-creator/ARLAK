"""
Windows auto-start management for ARLAK (V2).

Manages a single registry entry under
    HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run\\Arlak
which points at the silent launcher batch file (start-arlak-silent.bat) located
...
VALUE_NAME = "Arlak"
SILENT_LAUNCHER = "start-arlak-silent.bat"
...
def _ensure_launcher_exists() -> str:
    """
    Make sure start-arlak-silent.bat exists in the project root.
...
def enable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start enabled. Arlak will launch silently on next Windows login.",
        "enabled": True,
        "launcher": launcher,
...
def disable_auto_start(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": "Auto-start disabled. Arlak will no longer launch on login.",
        "enabled": False,
    }
...
def get_auto_start_status(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "result": (
            "Auto-start is ENABLED. Arlak launches on Windows login."
            if enabled
