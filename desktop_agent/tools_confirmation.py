...
Step 1: requestPowerAction(action) -> mints a single-use, short-lived token
        and tells ARLAK (via the result string) to ask the user to confirm.
...
# Friendly human labels so ARLAK's prompt-to-confirm reads naturally.
ACTION_LABEL = {
...
def request_power_action(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ARLAK calls this first; the returned message instructs her to ask the
