# ARLAK Desktop Control Agent

A local Python FastAPI service that gives ARLAK **JARVIS-style desktop control** —
...
> **This agent does NOT modify ARLAK's UI, personality, or chat system.** It is a pure
> backend tool layer that ARLAK's existing Node bridge (`server.ts`) calls over HTTP.
...
cd C:\Users\MSI\Desktop\arlak-ai-assistant
...
The agent binds to `127.0.0.1:8765`. Then start ARLAK normally with `npm run dev`.
...
| `saveScreenshot` | Save screenshot to Pictures/ArlakScreenshots |
...
- **Power actions** (shutdown, restart, sleep, lock) require a **two-step confirmation token**: ARLAK must first call `requestPowerAction` (which issues a single-use, 60-second token), ask the user out loud to confirm, then call `executePowerAction` with the token. Without a valid token, the action is refused.
...
```
ARLAK voice chat (existing, untouched)
        ↓
