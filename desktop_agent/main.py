"""
ARLAK Desktop Control Agent — FastAPI entrypoint.

Single dispatch endpoint POST /execute { tool, args } -> { result } | { error }.
ARLAK's Node bridge (server.ts) calls this over HTTP on 127.0.0.1:8765.
...
log = logging.getLogger("arlak.desktop")
...
async def lifespan(app: FastAPI):
    log.info("ARLAK Desktop Control Agent v%s starting up.", __version__)
    yield
    ...
    log.info("ARLAK Desktop Control Agent stopped.")


app = FastAPI(
    title="ARLAK Desktop Control Agent",
    version=__version__,
    description="JARVIS-style desktop automation backend for ARLAK.",
    lifespan=lifespan,
)
...
def health() -> Dict[str, Any]:
    return {
        "status": "ok",
        "name": "ARLAK Desktop Control Agent",
        "version": __version__,
