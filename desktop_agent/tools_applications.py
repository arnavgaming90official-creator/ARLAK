...
def _launch(spec: Dict[str, str]) -> None:
    try:
        if "exe" in spec:
            exe = spec["exe"]
            if shutil.which(exe) or os.path.isabs(exe):
...
def open_application(args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Fallback: search Start Menu shortcuts with fuzzy matching
    target_name = str(name).lower().replace('.exe', '').strip()
    start_menu_paths = [
    ...
    for sm_path in start_menu_paths:
        if not sm_path.exists(): continue
        for root, _, files in os.walk(str(sm_path)):
            for file in files:
                if file.endswith(".lnk"):
                    all_lnks.append((os.path.join(root, file), os.path.splitext(file)[0].lower()))
                    
    # 1. Try exact substring match first
    for path, base in all_lnks:
        if target_name in base:
            matches.append(path)
            
    # 2. If no substring match, try difflib fuzzy match
    if not matches:
        bases = [b for p, b in all_lnks]
        close = difflib.get_close_matches(target_name, bases, n=3, cutoff=0.5)
        if close:
            # find the paths for the close matches
            for path, base in all_lnks:
                if base in close:
                    matches.append(path)
                        
    if not matches:
        raise ToolError(f"Could not find application '{name}' in Start Menu or known aliases.")
    ...
    try:
        import subprocess
        subprocess.Popen(f'explorer "{best_match}"', shell=True)
        return {"result": f"Opened application from shortcut: {os.path.basename(best_match)}"}
