/* ===========================================================================
 * ARLAK — Electron main process (Phase 1)
 * ---------------------------------------------------------------------------
...
function startBackend() {
  ...
  const agentExe = app.isPackaged
    ? path.join(process.resourcesPath, 'agent', 'arlak-agent.exe')
    : path.join(APP_ROOT, 'agent_dist', 'arlak-agent', 'arlak-agent.exe');

  const env = {
    ...process.env,
    NODE_ENV: 'production',
    ELECTRON_RUN_AS_NODE: '1',
    ARLAK_LAUNCHED_BY: 'electron',
    ARLAK_DATA_DIR: dataDir,
    ARLAK_APP_ROOT: APP_ROOT,
  };
  if (fs.existsSync(agentExe)) {
    env.ARLAK_AGENT_EXE = agentExe;
  }
  ...
  serverProcess.on('exit', (code, signal) => {
    if (!isQuitting) {
      dialog.showErrorBox(
        'ARLAK backend stopped',
        `The ARLAK backend process exited unexpectedly (code ${code}, signal ${signal}).`,
      );
      app.quit();
    }
  });
...
async function bootstrap() {
  app.setAppUserModelId('com.arlak.desktop');
  createSplashWindow();
