...
export function formatSystemInstructionsWithMemories(baseInstruction: string, memories: Memory[]): string {
  ...
  let memoryBlock = 
    "\n\n" +
    "=== ARLAK PERSISTENT MEMORY CORE (RECOLLECTIONS) ===\n" +
...
export async function processConversationSlice(
  ...
  try {
    ...
    const dialogueContext = dialogueHistory.map(line => `${line.role === "user" ? "User" : "Arlak"}: ${line.text}`).join("\n");

    const prompt = `You are Arlak's deep cognitive recollection engine. Your task is to analyze the recent conversation piece against previous persistent memories, and output precise update transactions.
...
- ACTIONS:
  ...
  - "REMOVE": If a memory was explicitly disproven or the user directly asked Arlak to forget it.
- TEXT STYLE: Express the memories as clean, concise, third-person declarative summaries (e.g., 'The user is building a startup named Arlak.', 'The user loves playing GTA 6.', 'The user enjoys technical and fast-paced styling explanations.'). Do not include conversational filler, quotes, or timestamps.
