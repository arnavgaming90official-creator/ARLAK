...
// ---------------------------------------------------------------------------
// ARLAK Desktop Control Agent — HTTP bridge to the Python FastAPI backend.
// ---------------------------------------------------------------------------
...
const DESKTOP_TOOLS: ReadonlySet<string> = new Set([
  ...
  // files
  "createFile", "readFile", "renameFile", "moveFile",
  "openFolder", "listFiles", "searchFiles", "searchFilesAdvanced",
 ...
 * even if ARLAK's node process is killed.
 */
function spawnDesktopAgent(): void {
  const agentEnv = {
    ...process.env,
    ARLAK_AGENT_HOST: "127.0.0.1",
    ARLAK_AGENT_PORT: "8765",
  };

  // Preferred path (packaged app): a PyInstaller-frozen agent exe that embeds
  // its own Python runtime. Set by the Electron main process via ARLAK_AGENT_EXE.
  const frozenExe = process.env.ARLAK_AGENT_EXE;
  ...
  if (!py) {
    ...
    logError("AGENT_SPAWN_NO_RUNTIME: neither ARLAK_AGENT_EXE nor Python available");
    return;
  }
  try {
    const child = spawn(
...
async function startServer() {
  ...
  app.get("/api/web-proxy", async (req, res) => {
    ...
    try {
      ...
      if (!urlParam) {
        return res.status(400).send("Arlak Web Proxy Error: Missing target 'url' parameter");
      }
      ...
      if (targetUrl.startsWith("/")) {
        return res.status(400).send(`Arlak Web Proxy Error: Relative paths are not supported directly (${targetUrl}).`);
      }
      ...
      try {
        ...
        return res.status(400).send(`Arlak Web Proxy Error: Invalid URL specified: "${urlParam}". Make sure you enter a valid domain name.`);
      }
      ...
      try {
        ...
        return res.status(502).send(`Arlak Web Proxy Error: Unable to fetch the website "${targetUrl}". The site might be offline, or the URL address is spelled incorrectly. Details: ${fetchErr.message}`);
      }

      if (!response.ok) {
        return res.status(response.status).send(`Arlak Web Proxy Error: Failed loading remote website. Server returned status: ${response.status} (${response.statusText})`);
      }
      ...
      const interceptorScript = `
        <script>
          (function() {
            ...
            window.alert = function(msg) { console.log("[Arlak Browser alert bypassed]:", msg); };
            window.confirm = function(msg) { console.log("[Arlak Browser confirm bypassed]:", msg); return true; };
      ...
      res.setHeader("X-Arlak-Proxied", "true");
      ...
      res.status(500).send(`Arlak Web Proxy Error: Internal error occurred proxying URL "${targetUrl || "unknown"}". Details: ${e.message}`);
    }
  });
  ...
  wss.on("connection", async (clientWs) => {
    ...
    if (!apiKey) {
      ...
      clientWs.send(JSON.stringify({
        type: "error",
        error: "NO_API_KEY: Add your Gemini API key in Settings to start talking to ARLAK."
      }));
      clientWs.close();
      return;
    }
    ...
    try {
      ...
      const baseInstructions = 
        "You are Arlak, a warm, soft-spoken, and incredibly cute high-pitched anime heroine companion (age 18-22) holding an intimate, cozy voice call with TECH! Speak in a sweet, calm, polite, and affectionate anime-companion voice with a gentle, supportive, and slightly shy touch.\n" +
        ...
        "   - By default, when a user asks to open a website (like YouTube, Google, etc.), you MUST use the JARVIS-STYLE desktop agent tools (like 'openWebsite', 'searchYouTube', 'searchGoogle', 'searchWeb'). DO NOT use 'browserOpen' for these sites, as they block iframe embedding.\n" +
        "   - You must execute multi-step plans yourself! If the user says: 'Open YouTube and play Believer by Imagine Dragons', naturally confirm with your voice ('Sure thing, opening YouTube and starting Believer...') and IMMEDIATELY trigger 'searchYouTube' with query 'Believer by Imagine Dragons'. You do NOT need to wait for user instructions between these steps - chain them!\n" +
        "   - ONLY use 'browserOpen' (the internal holographic projector) if the user explicitly asks to 'open it in the projector', 'open it in your UI', or 'use your internal browser'.\n" +
        "8. TOOL TRIGGERS:\n" +
        "   - Use 'openWebsite' (Desktop Agent) to load webpages in the user's actual external browser (e.g. youtube.com, google.com, twitter.com).\n" +
        "   - Use 'browserOpen' ONLY for the internal holographic projector overlay when explicitly requested. Note: Many modern sites like YouTube and Google will refuse to load in this internal overlay due to security headers.\n" +
        "   - Use 'browserSearch' to search inside the active holographic projector.\n" +
        "   - Use 'browserClick' to click interactive buttons in the holographic projector.\n" +
        "   - Use 'browserMediaControl' to control media inside the holographic projector.\n" +
        "   - Use 'browserScroll' to scroll vertically in the holographic projector.\n" +
        "   - Use 'browserType' to write input fields in the holographic projector.\n" +
        "   - Use 'browserTabAction' to open, close, or focus tabs in the holographic projector.\n" +
        ...
        "   - APPLICATION CONTROL: Use 'openApplication' to launch ANY installed application by name (e.g., Notepad, Chrome, VS Code, Arduino IDE, etc.). It will automatically search your Start Menu if it doesn't immediately recognize it! Use 'closeApplication' to close them. Example: 'Open Arduino IDE' -> call openApplication(name='Arduino IDE') -> respond 'Arduino IDE opened.'\n" +
        ...
        "   - FILE MANAGEMENT: Use 'createFile', 'readFile', 'renameFile', 'moveFile', 'openFolder' (desktop/documents/downloads), 'listFiles', 'searchFiles', and 'searchFilesAdvanced'. Example: 'Create notes.txt on Desktop' -> createFile(path='Desktop/notes.txt'). 'Find my Python files' -> searchFiles(extension='py').\n" +
        "   - CRITICAL DISAMBIGUATION: If you search for a file or folder and multiple matches are returned, DO NOT guess which one to open. You MUST read the list of matches aloud to the user and ask them to clarify which one they meant.\n" +
        ...
        "   - DESKTOP BROWSER AUTOMATION (Playwright): Use the 'desktopBrowser*' tools to drive a REAL Chromium browser you own — open/navigate/search/click/type/fill forms/back/forward/scroll/open tab/close tab. This is separate from your holographic projector. Example: 'Fill in the login form on example.com' -> desktopBrowserOpen(url='example.com') then desktopBrowserFillForm(fields={...}). NOTE: Screen Share is READ-ONLY. You cannot blindly click on the user's OS browser via screen share. You MUST use desktopBrowser* tools to interact with web pages autonomously.\n" +
        "   - CODING ASSISTANCE & VS CODE: Use 'createPythonFile', 'writeCodeFile' (any language), 'createProjectFolder' (with subfolders), 'runPythonScript' (captures output). If the user asks you to edit code in VS Code, simply use 'writeCodeFile' or 'createFile' to modify the file on disk! VS Code will automatically reload the changes. Example: 'Create and run a hello world Python script' -> createPythonFile then runPythonScript, then read back the output naturally.\n" +
        ...
        "   - AUTO-START: Use 'enableAutoStart' when the user wants ARLAK to start with Windows, 'disableAutoStart' to remove it, 'getAutoStartStatus' to check. Explain what you're doing.\n" +
      ...
      const session = await ai.live.connect({
        ...
        config: {
          ...
          inputAudioTranscription: { languageCode: "en-US" } as any,
          outputAudioTranscription: { languageCode: "en-US" } as any,
          tools: [
            {
              functionDeclarations: [
                {
                  name: "browserOpen",
                  description: "Opens a designated website URL or interface tab inside Arlak's web agent console.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      url: {
                ...
                },
                {
                  ...
                  description: "Changes the visual theme or atmospheric glow color of Arlak's interface.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      color: {
                ...
                },
                {
                  ...
                  description: "Allows Arlak to immediately save a piece of critical user information to her persistent memory core.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      category: {
                ...
                {
                  name: "openWebsite",
                  description: "Open a named website shortcut (like 'youtube') or an arbitrary URL in the default OS browser. Do not use if the user asks to open in their 'specific' or 'current' active browser window.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      name: { type: Type.STRING, description: "Optional named shortcut (e.g., 'youtube', 'gmail')" },
                      url: { type: Type.STRING, description: "Optional raw URL to open (e.g., 'https://example.com')" }
                    }
                  }
                },
                {
                  name: "typeUrlInActiveBrowser",
                  description: "Use this ONLY when the user asks to open a website in their 'specific' or 'current' active browser window. Simulates Ctrl+L and types the URL. Requires the browser to be currently focused.",
                  parameters: {
                    type: Type.OBJECT,
                    properties: {
                      url: { type: Type.STRING, description: "Raw URL to type (e.g., 'https://youtube.com')" }
                    },
                    required: ["url"]
                  }
                },
                ...
                {
                  name: "searchFilesAdvanced",
                  description: "Find files or folders by fuzzy name, extension, and modification/creation date.",
                  parameters: { type: Type.OBJECT, properties: { folder: { type: Type.STRING, description: "Where to search (default 'home')." }, name: { type: Type.STRING, description: "Fuzzy name to match." }, extension: { type: Type.STRING, description: "File extension." }, type: { type: Type.STRING, description: "'file', 'folder', or 'both'." }, modified_after: { type: Type.STRING, description: "ISO date string." }, limit: { type: Type.INTEGER, description: "Max results (default 100)." } } }
                },
                {
                  name: "volumeUp",
                ...
                {
                  name: "saveScreenshot",
                  description: "Save a screenshot to Pictures/ArlakScreenshots.",
                ...
                // --- V2: Windows auto-start management ---
                {
                  ...
                  description: "Enable ARLAK to launch automatically when Windows starts. Creates a silent startup entry.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
                  name: "disableAutoStart",
                  description: "Disable ARLAK auto-start on Windows login. Removes the startup entry.",
                  parameters: { type: Type.OBJECT, properties: {} }
                },
                {
                  name: "getAutoStartStatus",
                  description: "Check whether ARLAK is currently configured to auto-start on Windows login.",
                  parameters: { type: Type.OBJECT, properties: {} }
                }
              ]
            }
        ...
        callbacks: {
          onmessage: (message: LiveServerMessage) => {
            ...
            const modelText = (message.serverContent as any)?.outputTranscription?.text;
            if (modelText) {
            ...
            const userTextOutput = (message.serverContent as any)?.inputTranscription?.text;
            if (userTextOutput) {
          ...
          onclose: (e: any) => {
            ...
            // If the connection was closed with an error code, bubble it to the UI
            if (e && e.code && e.code !== 1000 && e.code !== 1005) {
              clientWs.send(JSON.stringify({ 
                type: "error", 
                error: `Connection closed by Gemini (Code ${e.code}): ${e.reason || "Unexpected disconnect"}` 
              }));
            } else {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    logStartup(`ARLAK V2 server started on http://localhost:${PORT}`);
