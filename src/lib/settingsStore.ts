/**
 * ARLAK Settings Store — persistent user preferences (V2).
 *
 * Establishes the persistence pattern for ARLAK: settings are mirrored to
...
export interface ArlakSettings {
  /** Launch ARLAK (backends + browser tab) silently on Windows login. */
  autoStart: boolean;
  /** Enable the always-listening wake-word detector. */
  wakeWordEnabled: boolean;
  /** Phrase that activates ARLAK (case-insensitive substring match). */
  wakePhrase: string;
...
export const DEFAULT_SETTINGS: ArlakSettings = {
  autoStart: false,
  wakeWordEnabled: false,
  wakePhrase: "hey arlak",
  micDeviceId: "",
  sensitivity: 60,
  animations: true,
};

const STORAGE_KEY = "arlak.settings.v2";

/** Settings keys that the browser should never persist (security). */
const NEVER_PERSIST: ReadonlySet<keyof ArlakSettings> = new Set([]);
 ...
 */
export function loadSettings(): ArlakSettings {
  if (typeof window === "undefined") return { ...DEFAULT_SETTINGS };
  try {
    const raw = window.localStorage.getItem(STORAGE_KEY);
    if (!raw) return { ...DEFAULT_SETTINGS };
    const parsed = JSON.parse(raw) as Partial<ArlakSettings>;
 ...
 */
export function saveSettings(patch: Partial<ArlakSettings>): ArlakSettings {
  const current = loadSettings();
  const next: ArlakSettings = { ...current, ...patch };
  if (typeof window !== "undefined") {
    try {
      // Strip any sensitive keys before writing to localStorage.
      const safe: Record<string, unknown> = {};
      (Object.keys(next) as (keyof ArlakSettings)[]).forEach((k) => {
...
async function syncSettingsToBackend(settings: ArlakSettings): Promise<void> {
  try {
