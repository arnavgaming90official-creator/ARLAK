/**
 * Audio handling utility for Arlak Live API Voice stream.
 * Handles:
...
export class ArlakAudioSession {
  ...
  public async connect() {
    ...
    try {
      ...
      this.ws.onopen = async () => {
        console.log("[Arlak] Connected to server side WS bridge");
        try {
      ...
      this.ws.onmessage = async (event) => {
        try {
          ...
          if (data.type === "status") {
            console.log("[Arlak WS Status]:", data.status);
          ...
          // Handle interruption signal (e.g. user talked over Arlak)
          ...
          if (data.type === "turnComplete") {
            ...
            // Once Arlak completes speaking, change visual state back to listening
            setTimeout(() => {
