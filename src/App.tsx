...
import { ArlakCoreVisualizer, ArlakEmotion } from "./components/ArlakCoreVisualizer";
...
import { ArlakSettings, DEFAULT_SETTINGS, loadSettings, saveSettings } from "./lib/settingsStore";
import { ArlakWakeWordDetector } from "./lib/wakeWord";
...
export default function App() {
  ...
  const [activeEmotion, setActiveEmotion] = useState<ArlakEmotion>("idle");
  ...
  const detectEmotionFromText = (text: string): ArlakEmotion => {
  ...
  // Arlak Autopilot system controller state
  ...
  const wakeDetectorRef = useRef<ArlakWakeWordDetector | null>(null);
  ...
  useEffect(() => {
    const det = new ArlakWakeWordDetector();
  ...
  useEffect(() => {
    ...
    if (settings.wakeWordEnabled && state === "disconnected") {
      det.start({
        ...
        onTriggered: () => {
          // When wake word fires, stop detector and connect ARLAK.
          det.stop();
  ...
  const handleSettingsChange = (patch: Partial<ArlakSettings>) => {
    const next = saveSettings(patch);
    setSettings(next);
  };

  const sessionRef = useRef<ArlakAudioSession | null>(null);
  ...
  useEffect(() => {
    sessionRef.current = new ArlakAudioSession({
      ...
      onTranscription: (role, text) => {
        if (role === "user") {
          ...
          setModelCaption((prev) => {
            const next = turnCompletedRef.current ? text : prev + text;
  ...
  const getThemeTextGlow = () => {
    switch (themeColor) {
      ...
      case "emerald": return "text-emerald-400 drop-shadow-[0_0_12px_rgba(10,185,129,0.5)]";
  ...
  return (
    ...
    >
      ...
      {/* FULL VIEWPORT HOLOGRAPHIC STAGE: Arlak materializes across the entire screen */}
      ...
      <header className="relative z-30 flex items-center justify-between w-full max-w-5xl mx-auto select-none">
        ...
        <div className="flex items-center gap-5">
          ...
          {/* Real-time screen sharing toggler button inside Arlak glass style header */}
          <button 
      ...
      <main className="relative z-10 flex-1 w-full max-w-4xl mx-auto flex flex-col items-center justify-between py-6">
        ...
        {/* Text Chat Input Overlay */}
        {showChat && (
          <div className="w-full max-w-3xl px-6 relative z-30 mt-4 mb-2">
            <form 
              onSubmit={(e) => {
                e.preventDefault();
                if (chatInput.trim() && sessionRef.current) {
                  sessionRef.current.sendText(chatInput);
                  // Manually set user caption so they see what they typed immediately
                  setUserCaption(chatInput);
                  setModelCaption("");
                  setCharacterState("thinking");
                  setChatInput("");
                }
              }}
              className="relative flex items-center w-full bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-2xl backdrop-blur-xl focus-within:border-cyan-500/50 focus-within:bg-white/10 transition-all"
            >
              <input
                type="text"
                value={chatInput}
                onChange={(e) => setChatInput(e.target.value)}
                placeholder="Talk with Arlak..."
                className="w-full bg-transparent text-white font-sans text-sm sm:text-base px-6 py-4 outline-none placeholder:text-white/30"
              />
              <button 
                type="submit" 
                disabled={!chatInput.trim()}
                className="mr-2 px-4 py-2 bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/40 rounded-xl transition-colors disabled:opacity-30 disabled:cursor-not-allowed font-semibold text-xs tracking-wider font-mono"
              >
                SEND
              </button>
            </form>
          ...
          {showGuide && (
            ...
            >
              ...
              <p className="text-xs text-slate-400 mb-4 font-mono leading-relaxed">
                Arlak is equipped with dynamic visual modules and standard text browser projectors. Here are clever triggers to try speaking aloud:
              </p>
              <div className="space-y-2 text-xs font-serif italic text-indigo-300">
                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition cursor-pointer font-sans normal-case text-slate-200">
                  ⚡ &quot;Arlak, change atmosphere of your core to crimson&quot; <span className="text-[10px] font-mono text-indigo-400 block mt-0.5 font-medium">Shifts theme color background</span>
                </div>
                <div className="p-2.5 rounded-xl bg-white/5 border border-white/5 hover:bg-white/10 transition cursor-pointer font-sans normal-case text-slate-200">
      ...
      <footer className="relative z-10 w-full max-w-2xl mx-auto flex flex-col items-center gap-5 mt-auto">
        ...
        <div className="flex items-center justify-center relative mb-4">
          <button 
            ...
            title={state === "disconnected" ? "Awake Arlak" : "Sleep core"}
          >
            {state === "disconnected" ? (
